...
//									body.put("to_fcm_token", device.getDeviceFcmToken());
//									body.put("text", desc_temp);
//									body.put("file_name", upload.getName());
//									body.put("url", url_temp);
//									body.put("type", "video");
//									body.put("platform", "app");
//									body.put("time", (new Date()).getTime()/1000);
...
//				desc_temp.add(file_desc[i]);
				...
				if( user != null ){
					...
					for(int i = 0; i< deviceId.size(); i++){
						this.doPush(user, upload, deviceId.getInt(i), desc_temp, ConstantService.ossUrl + url, type);
					}
				}
				/*
				 * 推送在上
...
//        	logger.info("bad callback str: " + callbackBodyStr);
	...
	private void doPush(User user, Upload upload, Integer deviceId, JSONArray desc, String url, String type) throws UnsupportedEncodingException{
